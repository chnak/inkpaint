...
class TextMetrics {
  ...
  static measureText(text, style, wordWrap, canvas) {
    ...
    context.font = font; // const outputText = wordWrap
    ...
    if (wordWrap) {
      lines = TextMetrics.wordWrap(text, style, canvas); // console.log('measureText', text, lines);
    } else {
      lines = text.split(/(?:\r\n|\r|\n)/).map(line => line + "\n");
    }
    ...
    if (lastLine.endsWith('\n')) {
      after = -1; //lines[lines.length - 1] = lastLine.substring(0, lastLine.length - 1);
    }

    var before = Array.from(text).length;
    lines.map(l => after += Array.from(l).length);

    if (before !== after) {
      console.error('text parse err!!!', {
        before,
        after
      }, lines);
    } else {// console.log('text parse ok', {before, after}, lines);
    }
    ...
    for (var i = 0; i < lines.length; i++) {
      var mText = lines[i]; // 计算宽度的时候不考虑\n, 避免影响视觉居中

      if (mText.endsWith('\n')) mText = mText.substring(0, mText.length - 1);
      var lineWidth = context.measureText(mText).width + (Array.from(mText).length - 1) * style.letterSpacing;
  ...
  static wordWrap(text, style, canvas) {
    ...
    var tokens = TextMetrics.tokenize(text); // tokens是完整text, 一个字符都不少的
    // console.log('tokens', tokens);

    for (var i = 0; i < tokens.length; i++) {
      // get the word, space or newlineChar
      var token = tokens[i]; // console.log('-----token', {i, token});
      ...
      if (tokenWidth > wordWrapWidth) {
        ...
        if (TextMetrics.canBreakWords(token, style.breakWords)) {
          ...
          var characters = Array.from(token); // console.log('characters', characters);
          // loop the characters
          ...
          for (var j = 0; j < characters.length; j++) {
            ...
            line += char; // console.log('333 line += token', {line, token});
        ...
        if (tokenWidth + width > wordWrapWidth) {
          ...
          canPrependSpaces = false; // 一行末尾的空格，这里不加的话，下一行的开头也会丢弃

          while (TextMetrics.isBreakingSpace(token)) {
            line += token; // console.log('444 line += token', {line, token});
        ...
        if (line.length > 0 || !TextMetrics.isBreakingSpace(token) || canPrependSpaces) {
          ...
          line += token; // console.log('555 line += token', {line, token});
    ...
    lines.push(TextMetrics.addLine(line, 'T6')); // 把单个\n跟上一行结尾无\n的合并

    for (var _i = 0; _i < lines.length - 1; _i++) {
      if (lines[_i].endsWith('\n') || lines[_i + 1] !== '\n') continue;
      lines[_i] += lines[_i + 1];
      lines[_i + 1] = '';
    }
  ...
  static addLine(line, tag, newLine) {
    if (newLine === void 0) {
      newLine = true;
    }
