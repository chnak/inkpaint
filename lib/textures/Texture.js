...
class Texture extends _eventemitter.default {
  ...
  setCutoutColor(colorkey, colorsimilarity) {
    if (colorsimilarity === void 0) {
      colorsimilarity = 0.2;
    }

    if (typeof colorkey !== 'string' || !colorkey.startsWith('#')) return;
    var [h, s, l] = (0, _utils.rgb2hsl)(...(0, _utils.str2rgb)(colorkey));
    if (isNaN(h) || isNaN(s) || isNaN(l)) return;
    var ch = [Math.round(h - colorsimilarity * 200), Math.round(h + colorsimilarity * 200)];
    var cs = [25, 101];
    var cl = [1, 101]; // console.log('setCutoutColor', {h, s, l, ch, cs, cl});
