...
class BaseTexture extends _eventemitter.default {
  ...
  loadSource(source) {
    ...
    this.source = source; // source resources loaded
  ...
  adaptedNodeCanvas() {
    ...
    if (source && source instanceof _Image.default && source.isPsImage) {
      ...
      if (cutout) this.cutoutImageData({
        pixel: this.source,
        cutoutColors
      });
    } else if (cutout && source && source.constructor.name.includes('Canvas') && source.getContext) {
      var ctx = source.getContext('2d');
      var pixel = ctx.getImageData(0, 0, this.source.width, this.source.height);
  ...
  cutoutImageData(_ref) {
    ...
    var length = data.length; // todo: if simlarity = 0, use rgb equal
    ...
    for (var i = 0; i < length; i += 4) {
      ...
      if (ch[0] < h && h < ch[1] && cs[0] < s && s < cs[1] && cl[0] < l && l < cl[1]) {
        data[i + 3] = 0;
        ii++;
      }
    }
  ...
  update() {
    if (this.imageType !== "svg") {
      ...
      this.realHeight = this.source.naturalHeight || this.source.videoHeight || this.source.height; // update width and height
