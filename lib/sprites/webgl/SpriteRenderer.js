...
class SpriteRenderer extends _ObjectRenderer.default {
  ...
  onContextChange() {
    ...
    if (this.renderer.legacy) {
      ...
      this.MAX_TEXTURES = Math.min(gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS), _settings.default.SPRITE_MAX_TEXTURES); // step 2: check the maximum number of if statements the shader can have too..

      this.MAX_TEXTURES = (0, _checkMaxIfStatmentsInShader.default)(this.MAX_TEXTURES, gl);
    }

    this.shader = (0, _generateMultiTextureShader.default)(gl, this.MAX_TEXTURES); // create a couple of buffers

    this.indexBuffer = _pixiGlCore.default.GLBuffer.createIndexBuffer(gl, this.indices, gl.STATIC_DRAW); // we use the second shader as the first one depending on your browser may omit aTextureId
