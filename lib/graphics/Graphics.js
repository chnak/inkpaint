...
var _GraphicsData = _interopRequireDefault(require("./GraphicsData"));
...
var _bezierCurveTo = _interopRequireDefault(require("./utils/bezierCurveTo"));
...
class Graphics extends _Container.default {
  constructor(nativeLines) {
    if (nativeLines === void 0) {
      nativeLines = false;
    }
    ...
    this._localBounds = new _Bounds.default();
    /**
  ...
  lineStyle(lineWidth, color, alpha, alignment) {
    if (lineWidth === void 0) {
      lineWidth = 0;
    }
  ...
  bezierCurveTo(cpX, cpY, cpX2, cpY2, toX, toY) {
    ...
    (0, _bezierCurveTo.default)(fromX, fromY, cpX, cpY, cpX2, cpY2, toX, toY, n, points);
    this.dirty++;
    return this;
  }
  /**
  ...
  _renderSpriteRect(renderer) {
    ...
    if (!this._spriteRect) {
      this._spriteRect = new _Sprite.default(new _Texture.default(_Texture.default.EMPTY));
    }
  ...
  drawShape(shape) {
    ...
    this.currentPath = null;
    var data = new _GraphicsData.default(this.lineWidth, this.lineColor, this.lineAlpha, this.fillColor, this.fillAlpha, this.filling, this.nativeLines, shape, this.lineAlignment);
  ...
  generateTexture(renderer, scaleMode, resolution) {
    if (resolution === void 0) {
      resolution = 1;
    }

    var bounds = this.getLocalBounds(); // 留1px边框，避免贴着边导致 gl.CLAMP_TO_EDGE 效果

    var texture = _RenderTexture.default.create(bounds.width + 2, bounds.height + 2, scaleMode, resolution);
  ...
  generateCanvasTexture(scaleMode, resolution) {
    ...
    var canvasBuffer = _RenderTexture.default.create(bounds.width, bounds.height, scaleMode, resolution);

    if (!canvasRenderer) {
      canvasRenderer = new _CanvasRenderer.default();
    }
    ...
    var texture = _Texture.default.fromCanvas(canvasBuffer.baseTexture._canvasRenderTarget.canvas, scaleMode, "graphics");
  ...
  destroy(options) {
    super.destroy(options); // destroy each of the GraphicsData objects
